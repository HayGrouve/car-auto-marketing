...
Step-by-step for deploying this repo to the **client's Vercel account** on `stefi-gas.com`. Assumes migration spec ([spec-vercel-migration.md](./spec-vercel-migration.md)) is already merged.
...
All must pass. Build uses Nitro + TanStack Start (no Netlify).
...
Replace the current **Deploy to Netlify** section with **Deploy to Vercel**, summarizing steps 2–6 above in condensed form with the env var table and smoke checklist.
